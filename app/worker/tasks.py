...
async def notify_user(chat_id: int, domain_name: str, scan_results=None, error: str = None):
    if not chat_id:
        return
    ...
    try:
        if error:
            text = f"❌ <b>Scan Error: {domain_name}</b>\n\nReason: <code>{error}</code>"
        else:
            ips_count = len(scan_results) if isinstance(scan_results, list) else len(scan_results.get("ips", []))
            text = (
                f"✅ <b>Scan Complete: {domain_name}</b>\n\n"
                f"🔍 IP addresses found: <b>{ips_count}</b>\n"
                f"💾 Data successfully saved. Go to the domain menu to view the results."
            )
